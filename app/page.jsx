...
export default function Home() {
  ...
  useEffect(() => { setFiches(loadFiches()); }, []);
  ...
  function resetForm() {
    setFirstName(""); setLastName(""); setPhone(""); setLines([]);
  }
  ...
  function handleCreate() {
    ...
    setFiches(next); saveFiches(next); resetForm();
  }

  function handleDelete(id) {
    if (!confirm("Supprimer cette fiche ?")) return;
    const next = fiches.filter(f => f.id !== id);
    setFiches(next); saveFiches(next);
  ...
  function saveEdit() {
    ...
    setFiches(next); saveFiches(next); cancelEdit();
  }
  ...
  function exportJSON() {
    ...
    a.href = url; a.download = `rers_fiches_${new Date().toISOString().slice(0,10)}.json`; a.click();
  ...
  function importJSON(e) {
    ...
    reader.onload = () => {
      try {
        ...
        saveFiches(parsed); setFiches(parsed); alert("Import réussi ✅");
      } catch (err) { alert("Import invalide : " + err.message); }
    };
    reader.readAsText(file); e.target.value = "";
  }
  function reloadLocal() { setFiches(loadFiches()); }
  ...
  return (
    <main style={{ maxWidth: 960, margin: "24px auto", padding: 16 }}>
      ...
      <div style={{ display: "flex", gap: 8, alignItems: "center", margin: "8px 0 24px" }}>
        <button onClick={reloadLocal}>Recharger</button>
        <button onClick={exportJSON}>Exporter</button>
        <label style={{ display:"inline-flex", alignItems:"center", gap:8 }}>
          <span>Importer</span>
          <input type="file" accept="application/json" onChange={importJSON} />
        </label>
        <span style={{ marginLeft: "auto" }}>{fiches.length} fiche{fiches.length>1?"s":""}</span>
      </div>
      ...
      <section style={{ borderTop: "1px solid #ddd", paddingTop: 12, marginTop: 12 }}>
        ...
        {lines.map((l, i) => (
          <div key={i} style={{ display:"flex", gap:8, alignItems:"center", marginBottom:6 }}>
            ...
            <input value={l.text} onChange={(e)=>updateLine(i, e.target.value)} style={{ flex:1 }} />
